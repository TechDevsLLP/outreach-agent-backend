"""
Standalone onboarding + first campaign script for prithvi@techdevs.in.

Full flow (no sending):
  Account bootstrap → company profile (ICP) → synthetic campaign →
  run_fast_discovery → poll enrichment → verify ≥50 companies → export

Outputs (backend/scripts/results/prithvi_<ts>/):
  prospects.csv        — contact + AI intel + pitch + generated messages
  companies.csv        — ≥50 enrolled companies
  cost_report.csv/.md  — Apify (real $) + Gemini + OpenRouter (complete coverage)
  run_summary.json     — counts, timings, target-hit status

Usage:
    cd backend
    python3 scripts/onboard_and_campaign_prithvi.py
"""
...
logger = logging.getLogger("prithvi_campaign")
...
TIER1_INDUSTRIES = [
    # ICP vertical names (for Gemini sourcing prompt)
    ...
    # LinkedIn industry taxonomy labels that map to these verticals
    "Retail",
    "Consumer Goods",
...
async def reset_account_state(account_id: str) -> None:
    ...
    # Collect campaign IDs first so we can clean sourced_companies
...
async def upsert_company_profile(account_id: str, user_id: str) -> str:
    ...
    profile = {
        **COMPANY_PROFILE_DOC,
        "account_id": account_id,   # always string in company_profiles
        "user_id": user_id,
        "updated_at": now,
    }
    ...
    # Canonicalize ICP → industry_ids, country_codes, seniorities, employee_bands, title_query_vec
...
# ─── PHASE C: Synthetic campaign ──────────────────────────────────────────────

async def create_campaign(account_id: str, user_id: str, profile: dict) -> str:
    """Build and insert the synthetic campaign doc. Returns campaign_id str."""
    ...
    doc = {
        ...
        "created_by": user_id,
        "name": "Tier-1 Ad-Restricted Multi-Outlet — Maps SEO",
        "type": "smart",
        ...
        # Target: 50 companies × 2 prospects = 100 enrolled (with 1.25× buffer)
        "prospect_count_target": 125,
        "daily_caps": {"linkedin_connection": 20, "email": 20, "linkedin_inmail": 5},
        # Multi-outlet brands range from small regional chains to large enterprises
        ...
        # ── De-waste tuning (script-only; production campaigns omit these) ─────
        # scrape_depth 3/company (was 8) + buffer 1.5x (was 2.5x) → ~60% Apify cost cut
        "discovery_scrape_depth": 3,
        "discovery_dropout_buffer": 1.5,
        "discovery_enrollment_cap": 2,
        # 4 parallel Gemini sourcing batches → ~4x speed-up on sourcing wall-clock
        "discovery_sourcing_concurrency": 4,
        # Enable per-company news + competitor research (cached once, shared across prospects)
...
# ─── PHASE D: Run discovery + poll ────────────────────────────────────────────

async def run_discovery(campaign_id: str, account_id: str) -> None:
    """Run fast discovery and poll until campaign reaches awaiting_approval (or timeout)."""
    ...
    # Poll until campaign status = awaiting_approval OR all enrollments have message_gen_status done/failed
    ...
    while time.monotonic() < deadline:
        ...
        camp = await database.campaigns_collection.find_one({"_id": campaign_oid}, {"status": 1, "discovery_status": 1})
        camp_status = (camp or {}).get("status", "")
        disc_status = (camp or {}).get("discovery_status", "")

        # Count enrollment states
        ...
        logger.info(
            "Poll: campaign_status=%s disc_status=%s enrollments=%d pending_msg_gen=%d",
            camp_status, disc_status, total, pending_gen,
        )

        done = (camp_status == "awaiting_approval") or (disc_status == "completed" and pending_gen == 0 and total > 0)
        if done:
            logger.info("Enrichment complete: campaign=%s total_enrolled=%d", camp_status, total)
            break
    else:
        logger.warning("Enrichment poll timed out after %ds", ENRICHMENT_POLL_TIMEOUT_S)


# ─── PHASE E: Verify + iterate + mark primary ─────────────────────────────────

async def count_distinct_companies(campaign_id: str, account_id: str) -> int:
    """Count distinct companies among enrolled prospects for this campaign."""
    campaign_oid = ObjectId(campaign_id)
    account_oid = ObjectId(account_id)
    pipeline = [
        {"$match": {
...
async def mark_is_primary(campaign_id: str, account_id: str) -> int:
    """Set is_primary=True on highest-ai_score enrollment per company. Returns company count."""
    campaign_oid = ObjectId(campaign_id)
    account_oid = ObjectId(account_id)
    account_id_str = str(account_id)
    ...
    logger.info(
        "Marked %d primary enrollments across %d companies (%d total enrollments)",
...
async def build_cost_report(campaign_id: str, account_id: str) -> dict:
    """Aggregate Apify (real $) + LLM (estimated $) for this campaign."""
    campaign_id_str = str(campaign_id)
    account_id_str = str(account_id)
    account_oid = ObjectId(account_id)

    # Apify costs — keyed by campaign_id
    apify_rows = await database.apify_usage_collection.find(
        {"campaign_id": {"$in": [campaign_id_str, campaign_id]}}
    ).to_list(1000)

    # LLM costs — keyed by campaign_id OR account_id (embeddings have no campaign_id)
    or_pipeline = [
        {"$match": {"$or": [
...
def _get_msg_fields(msgs: dict | None, channel: str) -> dict:
    """Extract message fields given the enrollment's channel."""
    out = {
        "email_subject_a": "", "email_subject_b": "", "email_body": "",
        "connection_note": "", "inmail_subject": "", "inmail_body": "",
    }
    if not msgs:
        return out
    # Try primary channel sub-dict first, then top-level
    if channel == "email":
        sub = msgs.get("cold_email") or msgs.get("email") or {}
        out["email_subject_a"] = sub.get("subject_a", "")
        out["email_subject_b"] = sub.get("subject_b", "")
        out["email_body"] = sub.get("body", "")
    elif channel == "linkedin_connection":
        sub = msgs.get("linkedin_connection") or {}
        out["connection_note"] = sub.get("note", "")
    elif channel == "linkedin_inmail":
        sub = msgs.get("linkedin_inmail") or {}
        out["inmail_subject"] = sub.get("subject", "")
        out["inmail_body"] = sub.get("body", "")
    # Also check top-level keys for fallback
    if not any(out.values()):
        out["email_subject_a"] = msgs.get("subject_a", "")
        out["email_subject_b"] = msgs.get("subject_b", "")
        out["email_body"] = msgs.get("body", "")
        out["connection_note"] = msgs.get("note", "")
        out["inmail_subject"] = msgs.get("subject", "")
    return out
...
async def export_all(campaign_id: str, account_id: str, run_meta: dict, out_dir: Path) -> None:
    ...
    # Full join: enrollments → prospects → prospect_state → companies
    pipeline = [
        {"$match": {
            "campaign_id": campaign_oid,
            "account_id": {"$in": [account_oid, account_id]},
    ...
    # ── prospects.csv
    prospect_cols = [
        "is_primary", "enrollment_status", "ai_score", "priority_tier",
        ...
        "smart_campaign_channel", "smart_campaign_send_day",
        "enrolled_at", "message_gen_status",
        "email_subject_a", "email_subject_b", "email_body",
        "connection_note", "inmail_subject", "inmail_body",
    ...
    with open(out_dir / "prospects.csv", "w", newline="", encoding="utf-8") as f:
        ...
        for row in rows:
            ...
            st = row.get("state") or {}
            co = row.get("co") or {}
            msgs = row.get("generated_messages") or {}
            channel = row.get("smart_campaign_channel", "")

            # Merge intel: base (on prospect doc) + pitch (on prospect_state)
            base_intel = p.get("prospect_intelligence_base") or {}
            pitch = st.get("pitch") or {}
            intel = {**base_intel, **pitch}

            loc = p.get("location") or {}
            msg_fields = _get_msg_fields(msgs, channel)
    ...
    for row in rows:
        ...
        if key not in co_map:
            ...
            co_map[key] = {
                ...
                "employee_band": (
                    p.get("company_employee_band") or co.get("employee_band", "")
                ),
    ...
    co_cols = [
        "company_name", "domain", "industry_label", "employee_band", "employee_count",
    ...
    with open(out_dir / "companies.csv", "w", newline="", encoding="utf-8") as f:
        ...
        for co_row in sorted(co_map.values(), key=lambda x: x.get("company_name", "")):
            w.writerow(co_row)
    logger.info("Wrote companies.csv: %d rows", len(co_map))
    ...
    with open(out_dir / "cost_report.csv", "w", newline="", encoding="utf-8") as f:
        ...
        for li in cost["line_items"]:
            w.writerow(li)
    logger.info("Wrote cost_report.csv: %d line items", len(cost["line_items"]))

    md_lines = [
        "# Cost Report — Prithvi / TechDevs Campaign",
        "",
        ...
        "- Gemini sourcing tokens from `response.usage_metadata`; embedding tokens from usage_metadata or chars/4 estimate.",
...
async def main() -> None:
    ...
    out_dir = RESULTS_DIR / f"prithvi_{ts}"
    run_meta: dict = {
    ...
    logger.info("=== Phase A: Reset per-account state (keeping global pool) ===")
    await reset_account_state(account_id)

    # B: Company profile
    logger.info("=== Phase B: Upsert company profile + ICP canonicalization ===")
    ...
    # C: Campaign
    logger.info("=== Phase C: Create campaign ===")
    ...
    for iteration in range(MAX_DISCOVERY_ITERATIONS):
        ...
        try:
            ...
            logger.error("Discovery iteration %d failed after %.0fs: %s — stopping early", iteration + 1, elapsed, _disc_err)
    ...
    # Export
    logger.info("=== Phase F-H: Export to %s ===", out_dir)
