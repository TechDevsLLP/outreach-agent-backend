...
async def deduplicate_new_prospects(
    prospects_data: list[dict],
    industry_id: str,
    tags: Optional[list] = None,
) -> dict:
    """
    Process a batch of new prospects, deduplicating globally.

    For each prospect:
    - If exists: merge (add industry source, merge tags)
    - If new: create fresh

    Args:
        prospects_data: List of prospect dicts (must have email/linkedin)
        industry_id: Source industry ID
        tags: Tags to apply from this industry

    Returns:
        {
            "new": [new prospect dicts],
            "duplicates": [duplicate prospect dicts with merged industry],
            "stats": {"new": N, "duplicates": N}
        }
    """
    new_prospects = []
    duplicate_prospects = []
    ...
    for prospect_data in prospects_data:
        email = prospect_data.get("email", "").lower().strip()
        linkedin = prospect_data.get("linkedin", "")

        existing = await find_duplicate_prospect(email, linkedin)

        if existing:
            # Merge with existing
            updated = await merge_prospect_with_industry(
                str(existing["_id"]),
                industry_id,
                tags,
            )
            duplicate_prospects.append(updated)
        else:
            # Mark as new (set source industry)
            prospect_data["source_industry_id"] = industry_id
            prospect_data["source_industry_ids"] = [industry_id]
            if tags:
                prospect_data.setdefault("tags", [])
                prospect_data["tags"] = list(set(prospect_data["tags"] + tags))
            new_prospects.append(prospect_data)

    logger.info(
        f"Deduplication complete: {len(new_prospects)} new, "
        f"{len(duplicate_prospects)} duplicates merged"
    )

    return {
        "new": new_prospects,
        "duplicates": duplicate_prospects,
        "stats": {
            "new": len(new_prospects),
            "duplicates": len(duplicate_prospects),
